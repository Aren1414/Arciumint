...
pub struct MintNFT<'info> {
    ...
    /// CHECK: This is a PDA (unchecked) used as mint authority signer.
    pub mint_authority: UncheckedAccount<'info>,

    #[account(mut)]
    /// CHECK: Account checked by CPI call (Metaplex)
...
fn mint_token_to_user<'info>(ctx: &Context<MintNFT<'info>>, signer_seeds: &[&[u8]]) -> Result<()> {
